...
def load_masks(viewer, mask_path, h, w):
    epi_mask  = np.load(f"{mask_path}/epi.npy")
    endo_mask = np.load(f"{mask_path}/endo.npy")

    # Scale factors from saved mask size to current image size
    scale_row = h / epi_mask.shape[0]
    scale_col = w / epi_mask.shape[1]
...
def alpha_segment (root, data, patient, slice, methods, method):
    ...
    if mask_path is not None:
        load_masks(viewer, mask_path, h, w)
    ...
    save_masks(viewer, new_mask_path, h, w)
