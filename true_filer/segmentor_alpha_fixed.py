...
def save_masks(viewer, new_mask_path, h, w, ds):
    ...
    np.save(f"{new_mask_path}/pixel_spacing.npy",
            np.array(ds.PixelSpacing, dtype=float))
...
def load_masks(viewer, mask_path, ds_new):
    ...
    viewer.layers['endo'].data = mask_to_shapes(
        endo_mask, scale_row, scale_col)
...
def alpha_segment(root, data, patient, slice, methods, method):
    ...
    image_layer = viewer.add_image(
        image_data, name=f"/p{patient}/s{slice}/im/{methods[method]}.dcm")
